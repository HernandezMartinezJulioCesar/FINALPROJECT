...
void Model::Draw(Shader& shader, Camera& camera, glm::mat4 modelMatrix)
{
	...
	for (unsigned int i = 0; i < meshes.size(); i++)
	{
		...
		glUniformMatrix4fv(glGetUniformLocation(shader.ID, "rotation"), 1, GL_FALSE, glm::value_ptr(glm::toMat4(rotationsMeshes[i])));
		glUniformMatrix4fv(glGetUniformLocation(shader.ID, "scale"), 1, GL_FALSE, glm::value_ptr(glm::scale(glm::mat4(0.001f), scalesMeshes[i])));
